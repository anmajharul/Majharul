...
print("--- Shadow Clone Mode (closer clones) started ---")


# Clone placement function
...
while True:
    ...
    if effect_timer > 0:
        ...
        # Clone spacing (closer together)
        # Previous values were 0.35 and 0.60; reduced for tighter spacing
        dist1 = int(w * 0.15)  # inner clones
        dist2 = int(w * 0.30)  # outer clones
        offsets = [-dist2, -dist1, dist1, dist2]

        for off in offsets:
            # Scale set to 0.70 (70%)
            canvas = apply_clone(canvas, person_only, mask, 0.70, off)

        mask_3d = np.stack((mask,) * 3, axis=-1)
        # Keep the original subject in front of the clones
